...
| **Thuật ngữ**                              | **Ký hiệu/ký tự cú pháp**         | **Định nghĩa** |
|---|---|---|
| Vai trò (Role)                         | **`@N`**                          | Đơn vị tổ chức lớn nhất trong nội dung quy trình của sơ đồ làn bơi. Mỗi Giai đoạn thuộc về một Vai trò duy nhất. Các Giai đoạn tạo thành trục thời gian chính: G1, G2, G3, ... Không bắt buộc đối với sơ đồ không phải làn bơi. |
| Làn (Swimlane)                         | **`@N`**                          | Vùng riêng trên sơ đồ dành cho một Vai trò. Tên Vai trò được ghi làm nhãn của Làn. Các Nút của Vai trò đó được đặt bên trong Làn này. |
...
| Cổng vào (Start)                       | **`\.`**                          | Nơi bắt đầu vào của quy trình. Có thể có nhiều Cổng vào. Trên sơ đồ: hình elip hoặc bầu dục. |
| Cổng ra (End)                          | **`./`**                          | Nơi thoát khỏi quy trình (kết thúc). Có thể có nhiều Cổng ra. Trên sơ đồ: hình elip hoặc bầu dục. |
| Cổng phụ (Off-page Connector)          | **`\/`**                         | Ký hiệu thay thế cho Đường luồng nối sang trang sơ đồ khác. Thay thế cho Cổng vào hoặc Cổng ra. Mô tả của Cổng phụ nên ngắn gọn 1-2 ký tự (chữ số 0-9 hoặc chữ cái a-Z). Trên sơ đồ: hình ngũ giác ngược với cạnh nhọn quay xuống dưới. |
...
| Nhánh (Branch)                         |                             | Một Luồng đi ra từ một Nút (trừ Cổng ra, Cổng phụ đầu ra). Một nhánh có thể bao gồm một hoặc nhiều Nút. |
| Phân tách/song song (Fork/Parallel)    |                             | Từ một Nút (trừ Cổng ra, Cổng phụ đầu ra) chia ra nhiều Luồng thực hiện đồng thời (AND-fork). |
...
Một tài liệu quy trình hợp lệ theo bộ quy tắc này luôn bao gồm các thành phần sau, theo đúng thứ tự:

1. Khai báo Vai trò: xác định các bên tham gia quy trình.
2. Nội dung quy trình: gồm các Giai đoạn và Nút. Cổng vào/ra, Cổng phụ và Điểm nối được khai báo trực tiếp bên trong nội dung quy trình khi cần.

Các nguyên tắc cốt lõi cần nắm ngay:

- **QT.1.1:** Không được viết bất kỳ Giai đoạn hoặc Nút nào trước khi khai báo xong toàn bộ Vai trò.
...
- **QT.1.4:** Không được có Nút nào đứng ngoài một Giai đoạn. Mọi dòng `x.y` phải nằm sau một dòng `@N` `Gx` và trước dòng `@M B(x+1)` hoặc hết tài liệu.
...
- **QT.1.6:** Quy trình bắt đầu tại Nút 1.1 theo mặc định và kết thúc tại `./`.
- **QT.1.7:** Rẽ nhánh từ Quyết định tạo ra N Đường luồng nhưng chỉ một được chọn (XOR). Rẽ nhánh từ các Nút khác tạo ra Phân tách song song (AND-fork).
...
- **QT.1.12:** Mọi nhánh đều phải có đích xác định: một ID Nút khác hoặc Cổng ra `./`.
...
**1.3.2 Quyết định:** thêm dấu `?` sau ID, các nhánh đi ra được viết ở bên dưới, mỗi nhánh một dòng.
...
Vai trò là người hoặc bộ phận chịu trách nhiệm thực hiện một hoặc nhiều Giai đoạn trong quy trình. Việc khai báo Vai trò giúp xác định rõ ''ai làm gì''. Khi vẽ sơ đồ, mỗi Vai trò chiếm một Làn (swimlane) riêng biệt.
...
- **QT.2.1:** Dòng khai báo Vai trò phải xuất hiện ở đầu tài liệu, trước mọi Giai đoạn và Nút.
- **QT.2.2:** Mỗi Vai trò chiếm một dòng riêng.
- **QT.2.3:** Số `@N` phải tăng dần liên tục: `@1`, `@2`, `@3`, ... Không được viết `@1` rồi nhảy sang `@3`.
- **QT.2.4:** Trong phần nội dung quy trình, có thể tham chiếu đến một Vai trò bằng `@N` hoặc bằng tên viết tắt đã khai báo. Cả hai cách đều hợp lệ, nhưng trong cú pháp khai báo Giai đoạn (`@N` `Gx`) bắt buộc phải dùng `@N`.

### 2.4 Ví dụ

Ví dụ 1 - Có tên viết tắt:
...
Ví dụ 3 - Không hợp lệ vì thiếu `@2`:
...
- Mỗi Vai trò được vẽ thành một Làn (swimlane) hoặc một vùng riêng biệt trên sơ đồ.
...
### 2.6 Mẹo: Sơ đồ cá nhân hoặc cơ cấu tổ chức đơn giản

Đối với sơ đồ mô tả các bước thực hiện của một vai trò duy nhất (ví dụ: quy trình làm việc cá nhân, sơ đồ hướng dẫn từng bước, sơ đồ cơ cấu tổ chức đơn giản) thì không cần khai báo nhiều Vai trò. Chỉ cần khai báo tối thiểu như sau:

```
@1 0
```

Trong đó `@1 0` khai báo một Vai trò duy nhất với tên là `0` (số không). Khi tên Vai trò là `0`, sơ đồ sẽ được vẽ không có Làn bơi, các Nút được trình bày thẳng trên sơ đồ mà không có khung Làn bao quanh.

Nếu Vai trò duy nhất có tên khác `0` (ví dụ: `@1 Nhân viên`), sơ đồ vẫn được vẽ với một Làn bơi duy nhất mang tên đó.

Ví dụ 4 - Sơ đồ quy trình cá nhân không cần Làn bơi:
...
Ví dụ 5 - Sơ đồ quy trình cá nhân có hiển thị tên Làn:
...
- **QT.3.6:** Luồng mặc định giữa các Giai đoạn: sau khi hoàn thành Nút cuối cùng của Giai đoạn `Gx`, quy trình sẽ chuyển sang Giai đoạn G(x+1), bắt đầu từ Nút đầu tiên của Giai đoạn đó. Có thể thay đổi bằng cách dùng `->`.
...
- **QT.4.5:** Mô tả, Khối ghi chú (`~...~`) và Đường luồng (`->`) đứng sau ID của Nút thì dù nằm cùng dòng hoặc ở dòng riêng bên dưới đều gắn với Nút đó. Điều này có nghĩa là một Nút có thể được viết trải ra nhiều dòng.
...
- **QT.5.3:** Luồng mặc định đi tuần tự từ `x.y` đến x.(y+1) mà không cần viết `->` ra. Tuy nhiên, Luồng mặc định sẽ bị dừng khi gặp một Nút có `->` tường minh. Khi bị dừng, `x.y` chỉ đi đến các đích được khai báo tường minh.
- **QT.5.4:** Khi một Nút có bất kỳ đường luồng nào trỏ đến đích `0` (số không), toàn bộ luồng đi ra của Nút đó bị hủy, bao gồm các đường luồng trỏ đến đích khác được khai báo cùng lúc. Trên sơ đồ, Nút đó được vẽ bình thường nhưng không có mũi tên nào xuất phát từ nó.
...
- **QT.5.7:** Có thể sử dụng `__` thay cho `->` để tạo Đường nối. Đường nối có cùng ngữ nghĩa điều khiển luồng như Đường luồng, nhưng trên sơ đồ được vẽ không có đầu mũi tên.
...
- **QT.6.2:** Bắt buộc phải dùng `"..."` trong các trường hợp sau:
  ...
  - Nội dung của Quyết định (sau dấu `?`) nếu có nhiều dòng hoặc chứa ký tự cú pháp.
- **QT.6.3:** Nội dung bên trong `"..."` được giữ nguyên toàn bộ khoảng trắng và xuống dòng.
...
- **QT.7.1:** Chỉ và chỉ được phép đặt Ghi chú ở ngay sau ID và Mô tả của một Nút (ghi chú của Nút đó).
- **QT.7.2:** Nội dung bên trong `~...~` là văn bản thuần túy, không có giá trị cú pháp. Bên trong `~...~` không được chứa ký tự `~`.
...
- **QT.8.1:** Mọi quy trình đều cần có ít nhất một Cổng vào và ít nhất một Cổng ra. Cổng vào được ký hiệu bằng `\.` và Cổng ra được ký hiệu bằng `./`. Trên sơ đồ, cả hai đều được vẽ bằng hình elip.
- **QT.8.2:** Khi quy trình chỉ có một Cổng vào thì mặc định Cổng vào sẽ nằm phía trước Nút 1.1 của Giai đoạn G1 và thuộc cùng Làn với G1. Mô tả mặc định của Cổng vào là `Bắt đầu`. Không cần khai báo nếu không cần tùy chỉnh.
- **QT.8.3:** Khi quy trình chỉ có một Cổng ra thì mặc định Cổng ra sẽ nằm cùng Làn với Giai đoạn cuối cùng chứa `-> ./`. Mô tả mặc định của Cổng ra là `Kết thúc`. Chỉ cần ghi `./` trong nội dung quy trình.
- **QT.8.4:** `./` có thể xuất hiện nhiều lần trong cùng một tài liệu.
- **QT.8.5:** Khai báo Cổng vào/ra chỉ cần thiết khi:
  - Có nhiều Cổng vào hoặc Cổng ra;
  - Cần tùy chỉnh Mô tả;
  - Cần thêm Ghi chú;
  - Cần thêm Nhãn cho Đường luồng ra từ Cổng vào.
...
- 'Nhãn' `->` Đích: đường luồng đầu tiên từ Cổng, không bắt buộc nếu Nút đầu tiên là 1.1.
...
Cổng phụ thay thế cho Đường luồng nối sang trang sơ đồ khác. Có hai dạng: dạng đầu vào (thay thế Cổng vào) và dạng đầu ra (thay thế Cổng ra). Cho phép có nhiều Cổng phụ đầu vào và nhiều Cổng phụ đầu ra.
...
- `\/`: ký hiệu Cổng phụ.
- Mô tả: nhãn ngắn gọn 1-2 ký tự (chữ số 0-9 hoặc chữ cái a-Z), dùng để ghép cặp Cổng phụ đầu vào và đầu ra tương ứng với nhau trên các trang sơ đồ khác nhau. Bắt buộc phải có.
- ~Ghi chú~ và 'Nhãn': không bắt buộc.
- Cổng phụ đầu ra không được có 'Nhãn' và `->` vì nó là điểm cuối của quy trình.

Cách sử dụng luồng đi của Cổng phụ đầu vào giống Cổng vào, chỉ khác ở ký hiệu khai báo.

### 9.3 Quy tắc

- **QT.9.1:** Không bắt buộc Mô tả của các Cổng phụ cùng loại đầu vào hoặc cùng loại đầu ra trong một quy trình phải khác nhau.
- **QT.9.2:** Mô tả của Cổng phụ đầu vào bắt buộc phải khác với Mô tả của Cổng phụ đầu ra.
...
- 'Nhãn': nhãn của đường luồng dẫn sang Nút tiếp theo, bắt buộc.
- Đích: đích của nhánh đó.

### 11.3 Quy tắc

- **QT.11.1:** Một Quyết định phải có ít nhất hai nhánh.
- **QT.11.2:** Các nhánh của cùng một Quyết định không được trùng nhãn.
- **QT.11.3:** Quyết định có thể lồng nhau: một nhánh của Quyết định có thể dẫn đến một Quyết định khác. Tạo Quyết định lồng nhau bằng cách để một nhánh trỏ đến ID của một Quyết định khác.

### 11.4 Ví dụ

Ví dụ 1 - Quyết định nhị phân đơn giản:
...
Ví dụ 4 - Quyết định với ghi chú định nghĩa nhãn:
...
Ví dụ 5 - Quyết định với nội dung nhiều dòng:
...
- **QT.12.6:** Luồng tuần tự mặc định không chạy qua Điểm nối. Điều này có nghĩa: Các Nút khác có thể đi đến Điểm nối theo luồng mặc định (không cần viết `->`). Nhưng từ Điểm nối, người viết bắt buộc phải khai báo tường minh luồng đi ra bằng `->`. Nếu không khai báo thì Điểm nối sẽ không dẫn đến bất cứ Nút nào.
...
Ví dụ 3 - Không khai báo, không có luồng ra:
...
Phân tách cho phép từ một Nút nguồn tạo ra nhiều Luồng thực hiện đồng thời độc lập. Nút nguồn có thể là Hành động, Cổng vào, Cổng phụ đầu vào hoặc Điểm phát. 
...
- **QT.13.1:** Phân tách chỉ có thể xuất phát từ một Hành động, Cổng vào, Cổng phụ đầu vào hoặc Điểm nối Điểm phát. Quyết định không thể là gốc của phân tách.
- **QT.13.2:** Các luồng trong cùng một phân tách được phân cách nhau bằng dấu phẩy (`,`). Dấu `,` có thể đứng ngay sau đích hoặc cách một khoảng trắng. Tại đích cuối cùng, dấu `,` không bắt buộc nhưng nếu có cũng không bị coi là lỗi cú pháp.
- **QT.13.3:** Khi viết dạng nhiều dòng, không bắt buộc có dấu `,` ở cuối mỗi dòng.
- **QT.13.4:** Có thể dùng thụt lề để trình bày các Nút nằm trong cùng một nhánh, giúp sơ đồ dễ đọc hơn nhưng không ảnh hưởng đến cú pháp.
...
```
    ...
    Giải thích: Nhánh 2.2 kết thúc tại Cổng ra, nhánh 3.1 tiếp tục đến 4.1. Hai nhánh không gộp với nhau.
```

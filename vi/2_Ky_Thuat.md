...
| Cổng phụ (Off-page Connector)       | **`\/`**                  | Ký hiệu thay thế cho Đường luồng nối sang trang sơ đồ khác. Loại (đầu vào hay đầu ra) được xác định tự động: có `->` đi ra là đầu vào, không có là đầu ra. Trên sơ đồ: hình ngũ giác ngược với cạnh nhọn quay xuống dưới. |
...
| Điểm nối (On-page Connector)        | **`()`**                  | Cặp ký hiệu gồm một hoặc nhiều Điểm thu và một Điểm phát, dùng để thay thế Đường luồng dài trên cùng trang hoặc làm điểm gộp AND-join cho các nhánh. Trên sơ đồ: hình tròn nhỏ.          |
...
| Cổng phụ | `x.y \/ "Mô tả" ~Ghi chú~` | Mô tả cần ngắn gọn 1-2 ký tự (0-9 hoặc a-Z). |
| Điểm nối | `x.y () "Mô tả" ~Ghi chú~` | Mô tả cần ngắn gọn 1-2 ký tự (0-9 hoặc a-Z). |
...
- **QT.6.2:** Bắt buộc phải dùng `"..."` trong các trường hợp sau:
  1. Mô tả có nhiều dòng;
  2. Mô tả được viết xuống dòng riêng (ID ở dòng trên, mô tả ở dòng dưới);
  3. Mô tả chứa bất kỳ ký tự hoặc chuỗi nào trùng với ký hiệu cú pháp: (`->`, `./`, `#`, `,`, `'`, `~`);
...
- **QT.8.2:** Muốn hiển thị Cổng vào trên sơ đồ, phải khai báo tường minh Cổng vào (ký hiệu `\.`) hoặc Cổng phụ đầu vào (ký hiệu `\/`) tại Nút 1.1 của Giai đoạn G1. Không có Cổng vào nào được tạo tự động.
- **QT.8.3:** Sau Cổng vào hoặc Cổng phụ đầu vào đầu tiên tại 1.1, có thể khai báo thêm các Cổng vào/Cổng phụ ở các ID hợp lệ khác; mỗi Cổng là một Nút riêng và phải tuân thủ quy tắc đánh số Nút.
...
- **QT.8.6:** Không có yêu cầu bắt buộc về loại Nút cuối cùng của quy trình. Nút cuối có thể là Cổng ra, Cổng phụ đầu ra, hoặc bất kỳ loại Nút nào khác tùy người viết.
- **QT.8.7:** Khai báo tường minh Cổng ra chỉ cần thiết khi:
  1. Có nhiều Cổng ra;
  2. Cần tùy chỉnh làn hoặc Mô tả;
  3. Cần thêm Ghi chú.
...
- **Cổng phụ đầu vào:** là Cổng phụ mà từ nó có Đường luồng `->` đi ra, và không có Đường luồng nào trỏ vào nó. Dùng để thay thế Cổng vào trên trang tiếp theo.
- **Cổng phụ đầu ra:** là Cổng phụ mà có Đường luồng `->` trỏ vào nó, và từ nó không có Đường luồng nào đi ra. Dùng để thay thế Cổng ra ở cuối trang.

Cho phép có nhiều Cổng phụ đầu vào và nhiều Cổng phụ đầu ra. Không có Cổng phụ nào vừa nhận luồng vào vừa có luồng đi ra; nếu có, đây là lỗi. Mọi luồng đến hoặc đi từ Cổng phụ đều phải khai báo tường minh bằng `->`.
...
- **QT.9.2:** Mô tả của Cổng phụ đầu vào bắt buộc phải khác với Mô tả của Cổng phụ đầu ra trong cùng một quy trình.
...
- **QT.9.4:** Mọi luồng đi vào Cổng phụ đầu ra và mọi luồng đi ra từ Cổng phụ đầu vào đều phải khai báo tường minh bằng `->`. Không có luồng tuần tự mặc định vào hoặc ra từ Cổng phụ.
...
- Vẽ thanh fork (thanh ngang), từ Nút nguồn có một Mũi tên đến thanh fork, từ thanh fork có nhiều Mũi tên đi ra đến từng đích;
- Vẽ nhiều Mũi tên xuất phát thẳng từ Nút nguồn đến từng đích, không có thanh fork.
...
- Vẽ thanh join (thanh ngang), các Mũi tên từ mọi nhánh hội tụ vào thanh join, từ thanh join có một Mũi tên duy nhất đến đích;
- Vẽ các Mũi tên trỏ thẳng vào đích, không có thanh join.

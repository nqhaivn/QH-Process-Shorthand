...
- **QT.1.6:** Quy trình bắt đầu tại Nút 1.1 theo mặc định và kết thúc tại dòng cuối cùng có chứa `./`.
...
- **QT.5.3:** Luồng mặc định đi tuần tự từ `x.y` đến `x.(y+1)` mà không cần viết `->` ra. Tuy nhiên, Luồng mặc định bị dừng tại `x.y` trong hai trường hợp: 
	- `x.y` có `->` tường minh. Khi đó `x.y` chỉ đi đến các đích được khai báo tường minh; 
	- `x.(y+1)` là Điểm nối. Khi đó luồng mặc định không tự chạy vào Điểm nối, người viết phải khai báo tường minh `-> x.(y+1)` nếu muốn dẫn luồng vào đó.
...
- **QT.6.4:** Nội dung bên trong `"..."` được giữ nguyên toàn bộ khoảng trắng và xuống dòng.
- **QT.6.5:** `@N` có thể xuất hiện bên Trong Mô tả của bất kì Nút nào như một ký hiệu tham chiếu. Khi vẽ sơ đồ, mỗi @N trong Mô tả sẽ được thay thế tự động bằng tên viết tắt của Vai trò đó (nếu có khai báo tên viết tắt), hoặc bằng tên đầy đủ (nếu không có tên viết tắt).
...
- **QT.7.3:** `@N` có thể xuất hiện bên Trong Ghi chú của bất kì Nút nào như một ký hiệu tham chiếu. Khi vẽ sơ đồ, mỗi @N trong Ghi chú sẽ được thay thế tự động bằng tên viết tắt của Vai trò đó (nếu có khai báo tên viết tắt), hoặc bằng tên đầy đủ (nếu không có tên viết tắt).
...
- **QT.8.2:** Khi quy trình không khai báo rõ Cổng vào thì Cổng vào Mặc định sẽ nằm phía trước Nút 1.1 của Giai đoạn G1 và thuộc cùng Làn với G1. Mô tả của Cổng vào Mặc định là `Bắt đầu`. Không cần khai báo nếu không cần tùy chỉnh.
- **QT.8.3:** Khi quy trình không khai báo rõ Cổng ra thì Cổng ra Mặc định sẽ nằm cùng Làn với Giai đoạn cuối cùng chứa `-> ./`. Mô tả của Cổng ra Mặc định là `Kết thúc`. 
- **QT.8.4:** `./` có thể xuất hiện nhiều lần trong cùng một tài liệu. 
- **QT.8.5:** Tất cả các Nút có `-> ./` đều cùng dẫn đến Cổng ra Mặc định. Để trỏ đến Cổng ra khác, phải khai báo tường minh `x.y ./` và dùng `-> x.y` trong nội dung quy trình.
- **QT.8.6:** Khai báo Cổng vào/ra chỉ cần thiết khi:
...
```
    ...
    1.1 A -> 1.2, -> 2.1
    1.2 B -> 1.3
    1.3 ./ Kết thúc nhánh A        # Cổng ra này nằm trong Làn của @1

    @2 G2
    2.1 C -> 2.2
    2.2 D -> ./                     # Cổng ra mặc định, nằm trong Làn của @2

    Giải thích: 1.3 ./ được vẽ trong Làn @1 vì nó thuộc G1.
    Cổng ra mặc định từ 2.2 được vẽ trong Làn @2 vì G2 là Giai đoạn cuối có -> ./
```
...
- **QT.11.4:** Trong cùng một Quyết định, không được có hai nhánh cùng trỏ đến một đích. Mỗi nhánh phải dẫn đến một đích riêng biệt.
- **QT.11.5:** Không được dùng đích `0` trong bất kỳ nhánh nào của Quyết định.
...
Ví dụ 6 - Không hợp lệ (hai nhánh cùng đích):

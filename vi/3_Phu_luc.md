...
1. Khai báo Vai trò (`@1`, `@2`, `@3`, ...): xác định các bên tham gia, phải viết trước tất cả nội dung khác. Mỗi Vai trò phải được khai báo trong một dòng riêng.
...
Sau Nút cuối của Giai đoạn `Gx`, nếu không có `->` tường minh, luồng tự chuyển sang Nút đầu tiên của `G(x+1)`. Nếu có `->` tường minh thì luồng mặc định này bị hủy.
...
Mô tả, Ghi chú và `->` có thể viết trên nhiều dòng, tất cả đều gắn với Nút đó, tức là một Nút có thể trải ra nhiều dòng.
...
- `->` có thể đặt cuối dòng của Nút hoặc trên dòng riêng bên dưới. `->` trên dòng riêng chỉ hợp lệ khi dòng ngay phía trên (bỏ qua dòng trống, Mô tả, Ghi chú và Bình luận) là khai báo ID (`x.y`).
- Khi một Nút có bất kỳ đường luồng nào trỏ đến đích `0` (số không), toàn bộ luồng đi ra của Nút đó bị hủy, bao gồm các đường luồng trỏ đến đích khác được khai báo cùng lúc.
...
- `@N` trong Mô tả được tự động thay bằng tên viết tắt (hoặc tên đầy đủ nếu không có viết tắt) khi vẽ sơ đồ.
...
Ghi chú là thông tin bổ sung, hiển thị trên sơ đồ dưới dạng khung nét đứt nối với Nút, nhưng không tham gia Luồng. `@N` trong Ghi chú được tự động thay bằng tên viết tắt (hoặc tên đầy đủ nếu không có viết tắt) khi vẽ sơ đồ.
...
Ghi chú phải đứng ngay sau ID và Mô tả của Nút mà nó gắn vào.
...
- Tất cả `-> ./` đều dẫn đến Cổng ra Mặc định. Muốn trỏ đến Cổng ra khác (đã khai báo tường minh), phải dùng `-> x.y` với `x.y` là ID của Cổng ra đó.
...
Mô tả của Cổng phụ đầu vào và đầu ra bắt buộc phải khác nhau (để phân biệt chiều đi).

Không gian Mô tả của Cổng phụ và Điểm nối là độc lập, nếu cùng nội dung cũng không liên quan đến nhau.
...
Quyết định là điểm rẽ nhánh, tại đây Luồng chọn đúng một nhánh trong số các nhánh được liệt kê (XOR). Trên sơ đồ được vẽ bằng hình thoi. Mỗi Quyết định phải có ít nhất hai nhánh, các nhánh không được trùng nhãn và không được trỏ đến cùng một đích.

Quyết định không được dùng đích `0` ở bất kỳ nhánh nào.
...
Điểm nối là loại Nút duy nhất mà luồng tuần tự mặc định không dẫn đến hoặc đi ra. Người viết bắt buộc phải khai báo tường minh bằng `->`.

Giữa các Điểm nối cùng Mô tả không được có Đường luồng dẫn đến nhau.
...
Các luồng phân tách phân cách nhau bằng dấu phẩy `,` và phải trỏ đến những đích khác nhau.
...
`#` có thể ở dòng riêng hoặc cuối một dòng ghi chép quy trình — và phần bị cắt là từ `#` đến hết dòng đó, bất kể trước `#` có ký tự gì.
